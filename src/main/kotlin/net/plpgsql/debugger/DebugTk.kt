...
fun getConnection(project: Project, connectionPoint: DatabaseConnectionPoint): DatabaseConnection {
    return getFacade(
        project,
        connectionPoint,
        null,
...
fun searchFunctionByName(
    ...
    val ids = plArgs.filterValues { args ->
        var res = true
        val minimalCount = args.count { arg -> !arg.default }
        if (callValues.size < minimalCount || callValues.size > args.size) {
            res = false
        } else {
            //Map call values
            ...
            val check: PlBoolean? = fetchRowSet<PlBoolean>(
                plBooleanProducer(),
                query
            ) {
                run(connection)
            }.firstOrNull()
            res = check?.value ?: false
        }
        res
    }.map { it.key }
...
fun plNull(value: String) = value == "NULL"
